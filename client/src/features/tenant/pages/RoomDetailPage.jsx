...
import { 
  MapPin, Share2, Heart, MessageSquare, CheckCircle, Bed, Users, Maximize, Home, Compass, ChevronLeft, ShieldCheck, ShieldAlert, FileSignature, UploadCloud, Loader2
} from 'lucide-react';
          ...
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <input 
                  type="time" 
                  value={viewingTime} 
                  min={viewingDate === todayDate ? new Date().toTimeString().slice(0, 5) : undefined}
                  onChange={(e) => setViewingTime(e.target.value)} 
                  style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d5db' }}
                />
              </div>
            </div>
            {viewingDate && viewingTime && new Date(`${viewingDate}T${viewingTime}`) < new Date() && (
              <p style={{ color: '#ef4444', fontSize: '13px', marginTop: '8px', marginBottom: '0' }}>
                * Giờ xem không thể ở trong quá khứ.
              </p>
            )}
            <div className="modal-actions" style={{ marginTop: '16px' }}>
              ...
              <button 
                ...
                disabled={viewingDate && viewingTime && new Date(`${viewingDate}T${viewingTime}`) < new Date()}
                style={{ opacity: (viewingDate && viewingTime && new Date(`${viewingDate}T${viewingTime}`) < new Date()) ? 0.5 : 1, cursor: (viewingDate && viewingTime && new Date(`${viewingDate}T${viewingTime}`) < new Date()) ? 'not-allowed' : 'pointer' }}
              >
                {t('roomDetail.bookSchedule', 'Đặt lịch')}
              </button>
            </div>
          </div>
        </div>
